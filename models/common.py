...
class DenseLayer(nn.Module):
    def __init__(self, in_channels, out_channels, drop_rate=0.0) -> None:
        super(DenseLayer, self).__init__()
        self.drop_rate = drop_rate
        
        self.conv1 = Conv(in_channels, out_channels*4)
        
        self.bn2 = nn.BatchNorm2d(out_channels*4)
        self.relu2 = nn.SiLU(inplace=True)
        self.conv2 = nn.Conv2d(out_channels*4, out_channels, 
    ...
    def forward(self, x):
        ...
        if self.drop_rate > 0:
            out = nn.Dropout(self.drop_rate, inplace=False)(out)
...
class DenseBlock(nn.Module):
    def __init__(self, in_channels, growth_rate, num_layers, drop_rate=0.0):
        ...
        for _ in range(num_layers):
            layer = DenseLayer(in_channels, growth_rate, drop_rate)
            layers.append(layer)
